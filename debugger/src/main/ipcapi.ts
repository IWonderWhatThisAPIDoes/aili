...
export function setupIpcEvents(webProcess: WebContents, debuggerManager: DebuggerManager) {
    debuggerManager.onProcessExit.hook((pid, exitCode) =>
        webProcess.send(ipc.GDB_EXIT, pid, exitCode),
    );
    debuggerManager.onProcessError.hook((pid, error) => webProcess.send(ipc.GDB_ERROR, pid, error));
    debuggerManager.onProcessOutput.hook((pid, output) =>
        webProcess.send(ipc.GDB_OUTPUT, pid, output),
    );
}
...
export function setupIpcApi(debuggerManager: DebuggerManager) {
    ...
    ipcMain.handle(ipc.GDB_INPUT, (_, pid, input) =>
        implementation.sendInputToDebugger(pid, input),
    );
