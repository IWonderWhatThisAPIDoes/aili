 ...
 *              of the observer.
 */
export interface Hookable<T extends unknown[] = []> {
    /**
 ...
 * ```
 */
export class Hook<T extends unknown[] = []> implements Hookable<T> {
    ...
    hook(callback: Observer<T>): ObserverHandle {
        ...
        const wrappedCallback = (...args: T) => callback(...args);
 ...
 * ```
 */
export const NULL_HOOK: Hookable<unknown[]> = {
    hook: () => NULL_UNHOOK,
};
