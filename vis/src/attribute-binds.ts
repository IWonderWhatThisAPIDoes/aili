 ...
 */
export function getNumeric(numFilter: (v: number) => number | undefined, value: string): number | undefined {
    let numericValue: number;
    let filteredValue: number | undefined;
    if (Number.isFinite(numericValue = Number(value)) && (filteredValue = numFilter(numericValue)) !== undefined) {
 ...
 * ```
 */
export function numeric(numFilter: (v: number) => number | undefined, unit: string = ''): (attr: string) => string | undefined {
    return value => {
        let numericValue = getNumeric(numFilter, value);
